...
function drawCustomDesignBack(doc, x, y, imageBuffer, qrBuffer) {
  ...
  doc.text('Scanner pour voir mon profil', x + mm(BLEED + SAFE), captionY, { width: safeW, align: 'center' });
...
async function buildPrintSheet(doc, cards, options) {
  const baseUrl = ((options && options.baseUrl) || getBaseDomain()).replace(/^https?:\/\//i, '').replace(/\/+$/, '');
  ...
  for (const card of cards) {
    // The QR on the back of a finished card takes the recipient straight to that person's
    // own profile (its whole purpose as an NFC business card) — it's the generic /commander
    // QR (see the standalone order-form sheet, routes/admin.js "qr-formulaire") that's for
    // prospecting, not this one.
    const profileUrl = `https://${baseUrl}/c/${card.card_id}`;
    qrBuffers[card.id] = await QRCode.toBuffer(profileUrl, {
      width: 400,
      margin: 1,
      color: { dark: '#111111', light: '#FFFFFF' }
    });
    qrDataUrls[card.id] = await QRCode.toDataURL(profileUrl, {
      width: 260,
      margin: 1,
      color: { dark: '#111111', light: '#FFFFFF' }
    });
  ...
  if (styleCards.length > 0) {
    renderedFaces = await renderPhysicalCardBatch(styleCards, {
      baseUrl,
      qrCaptionHtml: 'Scanner pour<br>voir mon profil',
